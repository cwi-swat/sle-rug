...
AQuestion cst2ast(ql: Question q) {
  switch (q) {
      ...
      return ifblock(cst2ast(c), [ cst2ast(q_) | Question q_ <- qq], src=ql@\loc);
  	case (Question)`if(<Expr c>){<Question* qq>}else{<Question* qqs>}`: 
      return ifelseblock(cst2ast(c), [ cst2ast(q_) | Question q_ <- qq], [ cst2ast(qs) | Question qs <- qqs], src=ql@\loc);  
